...
export class Zookeeper {
  static async doCheck() {
    ...
    const zkok = child_process.execSync("echo \"ruok\" | nc localhost 2181 ; echo", { encoding: 'utf-8'});
  ...
  static async hydrate(core: Stack, rest: Rest) {
    try {
      ...
      return core.getZookeeper().withCheckResponse({ status: 'failed', error: e, rest });
    }
  }
}