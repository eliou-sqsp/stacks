  ...
  renderActionMenu(getLogs: () => void, onToggle: () => void) {
    ...
    return (
      <Menu>
        ...
        {disabled ? (
          <MenuItem icon="play" text="Enable" onClick={onToggle} />
        ) : (
          <MenuItem icon="pause" text="Disable" onClick={onToggle} />
        )}
      </Menu>
    );
  }
      ...
      <div className="stack">
        <div className="stack-control">
          ...
          <div className="actions">
            <Popover
              content={this.renderActionMenu(
                () =>
                  window.open(
                    'http://localhost:3001/logs/' + stackName,
                    '_blank',
                  ),
                () => {
        ...
        <div className="stack-headers">
          {columns.map((column) => {
            return (
              <div
                className={classNames('stack-header', column.property)}
                key={column.property}
              >
                {column.title}
              </div>
        ...
        <div className="stack-group">
          {stack.services &&
            stack.getServicesAsList().map((service: Service) => {
              ...
              if (!route) {
                throw new Error(
                  `Could not get service route for ${JSON.stringify(
                    stackRoute,
                  )}, ${service.name}`,
                );
              }
